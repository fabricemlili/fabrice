...
if __name__ == "__main__":
    ...
    # compet_type = sport_event["sport_event_context"]["competition"]["type"]
    # if compet_type != "singles":
    #     import json
    #     print(json.dumps(sport_event, indent=4))
    #     exit()
...
#         "https://discord.com/api/webhooks/1514614188314460310/ejHQrzA0AkDyhbgQKVlGy7adp6StnklUKqXnPSlvztA2zAX4s64Jn16gfKh_HfY_DWYP",
#         json=payload,
#         timeout=10
#     )
...
#     while True:
#         url = "https://api.sportradar.com/tennis/trial/v3/en/schedules/live/timelines_delta.json"

#         headers = {
#             "accept": "application/json",
#             "x-api-key": "j1DVKvLQxN8H2cOZlr5iK3XxgiLZmWOVmEyaQBPg"
#         }
...
#             print("No new data. Sleeping for 5 seconds...")
#             time.sleep(5)
#             continue

#         last_generated_at = generated_at

#         for timeline in data["sport_event_timeline_deltas"]:

#             sport_event_timeline = timeline["sport_event_timeline"]

#             sport_event_id = sport_event_timeline["id"]
#             sport_event_status = sport_event_timeline["sport_event_status"]

#             if sport_event_id not in match_states:
#                 match_states[sport_event_id] = sport_event_status
#             else:
#                 previous_status = match_states[sport_event_id]
#                 if sport_event_status != previous_status:
#                     match_states[sport_event_id] = sport_event_status
#                 else:
#                     continue
...
#             period_scores = [
#                 f"{score['home_score']} - {score['away_score']}"
#                 for score in sport_event_status.get("period_scores", [])
#             ]

#             game_state = sport_event_status.get("game_state", {})

#             serving = game_state.get("serving")

#             if serving == "home":
#                 game_score = (
#                     f"{game_state.get('home_score')} - "
#                     f"{game_state.get('away_score')}"
#                 )
#             else:
#                 game_score = (
#                     f"{game_state.get('away_score')} - "
...
#             tie_break = game_state.get("tie_break")
...
#                 f"🎯 **Sets:** `{', '.join(period_scores)}`\n"
#                 f"🎾 **Game:** `{game_score}`\n"
#                 f"👉 **Serving:** `{serving}`\n\n"
#                 f"📍 **Status:** `{match_status}`\n"
#                 f"💥 **Last point:** `{last_point_result}`"
#             )
